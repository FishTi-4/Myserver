...
int main(int argc, char *argv[])
{
    ...
    int nips = max(1, min((argc >= 6 ? stoi(argv[5]) : 2), 20));
    ...
    while (g_running)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            else if ((evs[j].events & EPOLLIN) && c->state == Conn::RECEIVING)
            {
                ...
                // Check if we got complete echo (prefix ~26 + MSG_LEN = 56 bytes)
                if (c->recv_buf.size() >= (size_t)(MSG_LEN + 20))
                {
                    c->cnt++;
                    g_msgs++;
