...
{
    cerr << "用法: " << prog << " <host> <port> <连接数> [保持时间秒] [源IP数] [--verify]\n"
         << "示例: " << prog << " 127.0.0.1 8080 10000 10 20 --verify\n"
...
int main(int argc, char *argv[])
{
    ...
    cfg.source_ips = (argc >= 6) ? stoi(argv[5]) : 1;
    ...
    for (int batch = 0; batch < cfg.total_connections; batch += batch_size)
    {
        ...
        for (int i = batch; i < batch_end; i++)
        {
            ...
            if (cfg.source_ips > 1)
            {
                ...
                string src_ip = "127.0.0." + to_string((i % cfg.source_ips) + 1);
                inet_pton(AF_INET, src_ip.c_str(), &local_addr.sin_addr);
            ...
            for (int j = 0; j < n; j++)
            {
                ...
                if (events[j].events & EPOLLOUT && c->state == Connection::SENDING)
                {
                    ssize_t sent = send(c->fd, c->send_buf.c_str() + c->send_offset,
                                        c->send_buf.size() - c->send_offset, MSG_NOSIGNAL);
                    if (sent > 0)
                    {
                        c->send_offset += sent;
                ...
                else if (events[j].events & EPOLLIN && c->state == Connection::RECEIVING)
                {
                    ...
                    if (r > 0)
                    {
                        ...
                        // 检查是否收到回声
                        if (c->recv_buf.find(c->send_buf) != string::npos)
                        {
                            echo_success++;
